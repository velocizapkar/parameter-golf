...
class Hyperparameters:
    ...
    # Thought bubbles
    thought_bubble_size = int(os.environ.get("THOUGHT_BUBBLE_SIZE", 0))  # B: number of thought tokens (0=disabled)
    # GPTQ calibration
...
class GPT(nn.Module):
    def __init__(
        ...
        self.thought_bubble_size = thought_bubble_size
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        if thought_bubble_size > 0:
            self.thought_emb = nn.Parameter(torch.zeros(thought_bubble_size, model_dim))
    ...
    def _insert_thought_bubbles(self, x: Tensor) -> Tensor:
        """Insert B thought bubble embeddings 64 positions before the end of the sequence."""
        B = self.thought_bubble_size
        if B <= 0 or x.size(1) < 64:
            return x
        bsz, seq_len, dim = x.shape
        sp = seq_len - 64  # insert before the last 64 tokens
        thought = self.thought_emb.to(dtype=x.dtype).unsqueeze(0).expand(bsz, -1, -1)
        return torch.cat([x[:, :sp], thought, x[:, sp:]], dim=1)

    def _remove_thought_bubbles(self, x: Tensor, real_seq_len: int) -> Tensor:
        """Remove B thought bubble positions, restoring real_seq_len output."""
        B = self.thought_bubble_size
        if B <= 0 or real_seq_len < 64:
            return x
        sp = real_seq_len - 64
        return torch.cat([x[:, :sp], x[:, sp + B:]], dim=1)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        TB = self.thought_bubble_size
        n = self.num_layers
        real_seq_len = input_ids.size(1)  # 2048-B real tokens
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x = self.smear(x)
        # Insert B thought bubbles 64 positions before the end -> 2048 total
        x = self._insert_thought_bubbles(x)
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # VE indexes by input_ids (real_seq_len) but transformer runs on 2048;
        # pad VE with zeros at thought bubble positions
        def _expand_ve(ve: Tensor | None) -> Tensor | None:
            if ve is None or TB <= 0 or real_seq_len < 64:
                return ve
            sp = real_seq_len - 64
            pad = torch.zeros(ve.size(0), TB, ve.size(-1), device=ve.device, dtype=ve.dtype)
            return torch.cat([ve[:, :sp], pad, ve[:, sp:]], dim=1)
        for i in range(self.num_encoder_layers):
            ve = _expand_ve(self._get_ve(i, input_ids, ve_cache))
        ...
        for i in range(self.num_decoder_layers):
            ...
            ve = _expand_ve(self._get_ve(bi, input_ids, ve_cache))
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
        # Remove thought bubble positions -> back to real_seq_len
        x = self._remove_thought_bubbles(x, real_seq_len)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        """Return logits (bsz, real_seq_len, vocab) without computing loss."""
        TB = self.thought_bubble_size
        n = self.num_layers
...
def main() -> None:
    ...
    # For thought bubbles, eval uses real_seq_len tokens per window
    effective_eval_seq_len = effective_eval_seq_len - args.thought_bubble_size
    val_seq_len = max(args.train_seq_len, effective_eval_seq_len + args.thought_bubble_size)
    ...
    base_model = GPT(
        ...
        thought_bubble_size=args.thought_bubble_size,
    ).to(device).bfloat16()
    ...
    if args.thought_bubble_size > 0:
        scalar_params.append(base_model.thought_emb)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, real_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
